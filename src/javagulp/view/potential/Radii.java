...
public class Radii extends JPanel implements Serializable {
	...
	public Radii(boolean usermin, String[] lbls) {
		...
		final G g = new G();
		int x = 0, y = 0;
		final int lblwidth = 45, txtwidth = 75, height = 20;
		...
		if (usermin)
			w = w * 2 + 5;
		final int h = height * lbls.length + 5 * (lbls.length - 1);
	...
	public void setRadiiEnabled(boolean flag) {
		if (txtrmin != null) {
			for (final JTextField field: txtrmin)
				field.setEnabled(flag);
		}
		for (final JTextField field: txtrmax)
			field.setEnabled(flag);
	}
