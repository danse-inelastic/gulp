...
public class LennardEpsilonSigma extends PotentialPanel implements Serializable {
	...
	private final String txtMinimum = g.html("c<sub>1</sub> = (n/(m-n))<br>c<sub>2</sub> = (m/(m-n))");
	private final String txtZero = g.html("c<sub>1</sub> = (n/(m-n))(m/n)<sup>m/(m-n)</sup><br>c<sub>2</sub> = (m/(m-n))(m/n)<sup>n/(m-n)</sup></html>");
	...
	private final JLabel defineSigmaAsLabel = new JLabel(g.html("define " + g.sigma + " as"));
	private final JLabel jwAB_2_1 = new JLabel(g.html(g.epsilon + " = 2 sqrt("
	...
	private final JLabel jwAB_3_1 = new JLabel(g.html(g.epsilon + " = sqrt("
			+ g.epsilon + "<sub>1</sub> " + g.epsilon + "<sub>3</sub>) <br>"
			+ g.sigma + " = (" + g.sigma + "<sub>1</sub> + " + g.sigma
			+ "<sub>2</sub>)/2"));
	private final JLabel lblEpsSig = new JLabel(g.html("E = " + g.epsilon
			+ "(c<sub>1</sub>(" + g.sigma + "/r)<sup>m</sup> - c<sub>2</sub>("
			+ g.sigma + "/r)<sup>n</sup>)"));
	private final JLabel lblEquation = new JLabel(txtMinimum);
	private final JLabel lblSetParameters = new JLabel("set parameters by");
	...
	private final JPanel combinationRules = new JPanel();
	private final JScrollPane scrollPane = new JScrollPane();
	private final JPanel geometricCombinationRules = new JPanel();
	private final JPanel manualInputES = new JPanel();

	private final SerialListener keyEpsSigParameters = new SerialListener() {
		private static final long serialVersionUID = 9127812535462595754L;
		@Override
		public void actionPerformed(ActionEvent e) {
			try {
				final Lennard l = (Lennard) getParent().getParent();
	...
	public LennardEpsilonSigma clone() {
		final LennardEpsilonSigma l = new LennardEpsilonSigma();
