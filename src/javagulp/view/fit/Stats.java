...
public class Stats {
	...
	public class Something implements Comparable<Something> {
		...
		public int compareTo(Something o) {
			if (compareValues) {
				final Double a = new Double(this.value);
				final Double b = new Double(o.value);
				return a.compareTo(b);
			} else {
				final Double a = new Double(this.zScore);
				final Double b = new Double(o.zScore);
				return a.compareTo(b);
			}
		}
	...
	public void recalculate() {
		...
		for (int i=0; i < values.size(); i++) {
			final Something s = values.get(i);
	...
	public void removeOutliers() {
		...
		final int numberToRemove = (int) Math.round(percentOutliers / 100.0 * values.size());
	...
	public void findPopulations() {
		...
		try {
			for (int i=0; i < values.size()-1; i++) {
				final double a = values.get(i).value;
				//System.out.println("a " + a);
				boolean skip = true;

				if (pops.size() > 0) {
					final ArrayList<Something> temp = pops.get(pops.size()-1);
					final double b = temp.get(temp.size()-1).value;
					//System.out.println("b " + b);
					double avg = 0;
					for (final Something s: temp)
						avg += s.value;
					avg /= temp.size();
					final double change = Math.abs((a-avg)/a);
				...
				if (skip) {
					final double b = values.get(i+1).value;
					//System.out.println("b " + b);
					final double change = Math.abs((a-b)/a);
					//System.out.println("change " + change);
					if (change < 0.05) {//if values are within 5% of each other
						//add both to a new population
						final ArrayList<Something> temp = new ArrayList<Something>();
						temp.add(values.remove(i));
						temp.add(values.remove(i));
						pops.add(temp);
						i--;
		...
		System.out.println();
		for (final ArrayList<Something> p: pops) {
			for (final Something s: p) {
