...
public class MDRestartInit extends JPanel implements Serializable {
	...
	private final DefaultListModel CoordlistFixed = new DefaultListModel();
	private final TreeSet<Integer> fixedAtomslist = new TreeSet<Integer>();

	private final JTextField txtcurrenttime = new JTextField();

	private final JLabel lblstartingpoint = new JLabel("starting point in previous simulation (ps)");

	private final TitledPanel pnlrestarttime = new TitledPanel();
	private final RestartConstraint pnlrestartconstraint = new RestartConstraint();
	private final RestartCellAverages pnlrestartcellaverages = new RestartCellAverages();
	private final RestartAverages pnlRestartAverages = new RestartAverages();
	private final MDInit pnlMDInitialization = new MDInit();

	private class RestartConstraint extends TitledPanel {

		private static final long serialVersionUID = -1429809272279764418L;

		private final JLabel lblLambdaR = new JLabel(g.html("sum " + g.lambda
				+ "<sub>R</sub> (distance constraint)"));
		private final JLabel lblLambdaV = new JLabel("<html>sum " + g.lambda
				+ "<sub>V</sub> (velocity constraint)</html>");

		private final JTextField txtcfaverlambdaR = new JTextField();
		private final JTextField txtcfaverlambdaV = new JTextField();
	...
	private class MDInit extends TitledPanel {
		...
		private final JButton btnInitVelocities = new JButton("initialize velocities");
		private final JButton btnInitAccelerations = new JButton("initialize accelerations");
		private final JCheckBox chkCausesTheInitial = new JCheckBox("<html>initialise the velocites of atoms within a <br>molecule to the same average value, resulting in <br>no internal kinetic energy</html>");

		private final KeywordListener keyCausesTheInitial = new KeywordListener(chkCausesTheInitial, "nomolecularinternalke");
		...
		private void openJPad(JTextArea jTextArea) {
			final Nutpad nut = new Nutpad(jTextArea.getText());
	...
	private class RestartAverages extends TitledPanel {
		...
		private final JLabel lblsumv2 = new JLabel("<html>sum v<sup>2</sup></html>");
		private final JLabel lblsumofenergies = new JLabel("sum of energies");
		private final JLabel lblsumofvirials = new JLabel("sum of virials");
		private final JLabel lblsumoftemperatures = new JLabel("sum of temperatures");
		private final JLabel lblsumofconstraints = new JLabel("sum of constraints");
		private final JLabel lblsumofconstants = new JLabel("sum of constants");
		private final JLabel lblaveragingpoints = new JLabel("number of averaging points");

		private final JTextField txtaverv2 = new JTextField();
		private final JTextField txtaverenergies = new JTextField();
		private final JTextField txtavervirials = new JTextField();
		private final JTextField txtavertemp = new JTextField();
		private final JTextField txtaverconstraints = new JTextField();
		private final JTextField txtaverconstants = new JTextField();
		private final JTextField txtavernumPoints = new JTextField();
	...
	private class RestartCellAverages extends TitledPanel {
		...
		private final JLabel lblsuma = new JLabel("sum a");
		private final JLabel lblsumb = new JLabel("sum b");
		private final JLabel lblsumc = new JLabel("sum c");
		private final JLabel lblsumalpha = new JLabel(g.html("sum " + g.alpha));
		private final JLabel lblsumbeta = new JLabel(g.html("sum " + g.beta));
		private final JLabel lblsumgamma = new JLabel(g.html("sum " + g.gamma));
		private final JLabel lblsumcellvolumes = new JLabel("sum cell volumes");

		private final JTextField txtcavera = new JTextField();
		private final JTextField txtcaverb = new JTextField();
		private final JTextField txtcaverc = new JTextField();
		private final JTextField txtcaveralpha = new JTextField();
		private final JTextField txtcaverbeta = new JTextField();
		private final JTextField txtcavergamma = new JTextField();
		private final JTextField txtcavercellvolume = new JTextField();
